  ...
  return (
    <div>
      ...
      {eventsData.map((event, index) => (
        <div
          key={event.id}
          className={`rounded-lg shadow-sm overflow-hidden mb-4 flex flex-col md:flex-row ${index % 2 === 0 ? "bg-[#EEEAE6]" : "bg-[#DFF0EC]"}`}
        >
          {/* Event Image */}
          ...
          <div className="flex-1 p-6">
            <div className="flex items-start justify-between mb-3">
              ...
              <span
                className={`text-sm px-3 py-1 rounded-md ${event.event_type === "Online" ? "bg-[#DCFCE7] text-[#16A34A]" : "bg-[#BFDBFE] text-[#1E40AF]"}`}
              >
                {event.event_type}
              </span>
            </div>
            ...
            <div className="flex items-center gap-4 text-sm text-gray-800 mb-3">
              <span className="flex items-center gap-1">
                <Calendar strokeWidth={3.0} size={14} />
                {event.date}
              </span>
              <span className="flex items-center gap-1">
                <Clock strokeWidth={3.0} size={14} />
                {event.time}
              </span>
            </div>

            <div className="flex items-center gap-1 text-sm text-gray-800 mb-4">
              <MapPin strokeWidth={3.0} size={14} />
